function errchecking(evt) {
    ...
    if (gradeinput > 100 || gradeinput < 0) {
      evt.currentTarget.style = "background-color:tomato;";
    } else {
...
function calculateGrade() {
    ...
    if (uparticipationPercent < 0 || uparticipationPercent > 100 ) {
        document.getElementById("uparticipationPercent").style = "background-color:tomato;";
        isError = true;
    } 
    if (isNaN(uparticipationPercent)){
        uparticipationPercent = 0;
    }
    if (uhwPercent < 0 || uhwPercent > 100 ) {
        document.getElementById("uhwPercent").style = "background-color:tomato;";
        isError = true;
    }
    if (isNaN(uhwPercent)){
        uhwPercent = 0;
    ...
    if (uquizPercent < 0 || uquizPercent > 100 ) {
        document.getElementById("uquizPercent").style = "background-color:tomato;";
        isError = true;
    } 
    if (isNaN(uquizPercent)){
        uquizPercent = 0;
    } 
    if (upm1Percent < 0 || upm1Percent > 100 ) {
        document.getElementById("upm1Percent").style = "background-color:tomato;";
        isError = true;
    } 
    if (isNaN(upm1Percent)){
        upm1Percent = 0;
    } 
    if (upm2Percent < 0 || upm2Percent > 100 ) {
        document.getElementById("upm2Percent").style = "background-color:tomato;";
        isError = true;
    }
    if (isNaN(upm2Percent)){
        upm2Percent = 0;
    } 
    if (!isError){
